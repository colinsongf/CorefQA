...
class CorefModel(object):
    ...
    def coarse_to_fine_pruning(self, top_span_emb, top_span_mention_scores, c):
        ...
        if self.config['use_prior']:
            ...
            distance_scores = util.projection(
                tf.nn.dropout(tf.get_variable("antecedent_distance_emb", [10, self.config["feature_size"]]),
                              self.dropout), 1, initializer=tf.truncated_normal_initializer(stddev=0.02))  # [10, 1]
    ...
    def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts,
        ...
        candidate_starts, candidate_ends = tf.clip_by_value(tf.concat([gold_starts, [0]], -1), 0,
                                                            num_words - 1), tf.clip_by_value(
            tf.concat([gold_ends, [0]], -1), 0, num_words - 1)
    ...
    def get_mention_scores(self, span_emb, span_starts, span_ends):
        ...
        if self.config['use_prior']:
            span_width_emb = tf.get_variable("span_width_prior_embeddings",
                                             [self.config["max_span_width"], self.config["feature_size"]])  # [W, emb]
